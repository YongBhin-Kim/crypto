'''
    ARIA - Python Implementation
'''
# Copyright © Yonhbhin Kim
# Github url : https://github.com/?


'''
Constant for key expansion
'''
C1 = [0x51, 0x7c, 0xc1, 0xb7, 0x27, 0x22, 0x0a, 0x94, 0xfe, 0x13, 0xab, 0xe8, 0xfa, 0x9a, 0x6e, 0xe0]
C2 = [0x6d, 0xb1, 0x4a, 0xcc, 0x9e, 0x21, 0xc8, 0x20, 0xff, 0x28, 0xb1, 0xd5, 0xef, 0x5d, 0xe2, 0xb0]
C3 = [0xdb, 0x92, 0x37, 0x1d, 0x21, 0x26, 0xe9, 0x70, 0x03, 0x24, 0x97, 0x75, 0x04, 0xe8, 0xc9, 0x0e]


'''
Sbox1, Sbox2, Inverse of Sbox1, Inverse of Sbox2 for substlayer
'''
sbox = [[
  
],
[ 
]]

isbox = [[
  
],
[
  
]]


'''
Print list
'''
def printstate(msg, state, bytelen):
    print(msg, end='')
    for i in range(bytelen):
        print( '%02x ' %(state[i]), end='' )
    print()


'''
difflayer function
'''
def difflayer(src, dst):
    temp = src[:]

    dst[0]=temp[3]^temp[4]^temp[6]^temp[8]^temp[9]^temp[13]^temp[14]
    dst[1]=temp[2]^temp[5]^temp[7]^temp[8]^temp[9]^temp[12]^temp[15]
    dst[2]=temp[1]^temp[4]^temp[6]^temp[10]^temp[11]^temp[12]^temp[15]
    dst[3]=temp[0]^temp[5]^temp[7]^temp[10]^temp[11]^temp[13]^temp[14]

    dst[4]=temp[0]^temp[2]^temp[5]^temp[8]^temp[11]^temp[14]^temp[15]
    dst[5]=temp[1]^temp[3]^temp[4]^temp[9]^temp[10]^temp[14]^temp[15]
    dst[6]=temp[0]^temp[2]^temp[7]^temp[9]^temp[10]^temp[12]^temp[13]
    dst[7]=temp[1]^temp[3]^temp[6]^temp[8]^temp[11]^temp[12]^temp[13]

    dst[8]=temp[0]^temp[1]^temp[4]^temp[7]^temp[10]^temp[13]^temp[15]
    dst[9]=temp[0]^temp[1]^temp[5]^temp[6]^temp[11]^temp[12]^temp[14]
    dst[10]=temp[2]^temp[3]^temp[5]^temp[6]^temp[8]^temp[13]^temp[15]
    dst[11]=temp[2]^temp[3]^temp[4]^temp[7]^temp[9]^temp[12]^temp[14]
	
    dst[12]=temp[1]^temp[2]^temp[6]^temp[7]^temp[9]^temp[11]^temp[12]
    dst[13]=temp[0]^temp[3]^temp[6]^temp[7]^temp[8]^temp[10]^temp[13]
    dst[14]=temp[0]^temp[3]^temp[4]^temp[5]^temp[9]^temp[11]^temp[14]
    dst[15]=temp[1]^temp[2]^temp[4]^temp[5]^temp[8]^temp[10]^temp[15]
        

'''
Odd round F function
'''
def fo(state, key):
    
    # AddRoundKey
    for i in range(16):
        state[i] ^= key[i]

    # SubstLayer type 1 (LT||LT||LT||LT)
    for i in range(4):
        state[4*i]   = sbox[0][state[4*i]]      # s1
        state[4*i+1] = sbox[1][state[4*i+1]]    # s2
        state[4*i+2] = isbox[0][state[4*i+2]]   # inv s1
        state[4*i+3] = isbox[1][state[4*i+3]]   # inv s2
        

    # DiffLayer
    difflayer(state, state)


'''
Even round F function
'''
def fe(state, key):
    
    # AddRoundKey
    for i in range(16):
        state[i] ^= key[i]
    
    # SubstLayer type 2 (invLT||invLT||invLT||invLT)
    for i in range(4):
        state[4*i]   = isbox[0][state[4*i]]     # inv s1
        state[4*i+1] = isbox[1][state[4*i+1]]   # inv s2
        state[4*i+2] = sbox[0][state[4*i+2]]    # s1
        state[4*i+3] = sbox[1][state[4*i+3]]    # s2

    # DiffLayer
    difflayer(state, state)
    

'''
Final round F function
'''
def ff(state, key, whiteningkey):
    
    # AddRoundKey
    for i in range(16):
        state[i] ^= key[i]
    
    # SubstLayer type 2 (invLT||invLT||invLT||invLT)
    for i in range(4):
        state[4*i]   = isbox[0][state[4*i]]     # inv s1
        state[4*i+1] = isbox[1][state[4*i+1]]   # inv s2
        state[4*i+2] = sbox[0][state[4*i+2]]    # s1
        state[4*i+3] = sbox[1][state[4*i+3]]    # s2
        
    # AddRoundKey - Whitening key
    for i in range(16):
        state[i] ^= whiteningkey[i]
        

'''
Bits rotations function for key expansion
'''
def rot(input, bits):
    ret = [0]*16
    
    q = bits // 8
    r = bits % 8
    for i in range(16):
        ret[(q+i) % 16] ^= (input[i] >> r)
        if (r != 0):
            ret[(q+i+1) % 16] ^= ((input[i] << (8-r)) &0xfe)
    return ret


'''
Key expansion function for generate round key or whitening key
'''
def keyexpansion(mk, enc_w, dec_w):
    
    '''
    Key expansion initial part (ARIA128/ ARIA192/ ARIA256)
    '''
    W1, W2, W3 = [0]*16, [0]*16, [0]*16 # tuple unpacking
    KL = mk[:16]
    
    keylen = len(mk)*8
    if keylen == 128:
        KR = [0]*16
        CK1 = C1
        CK2 = C2
        CK3 = C3
    elif keylen == 192:
        KR = mk[16:] + [0 for _ in range(8)]
        CK1 = C2
        CK2 = C3
        CK3 = C1
    elif keylen == 256:
        KR = mk[16:]
        CK1 = C3
        CK2 = C1
        CK3 = C2
    else:
        print("Key length must be 128/192/256bits ")
    
    
    # Create W0, W1, W2, W3 (not parallel, fixed 16byte list)
    W0 = KL[:]
    
    temp = W0[:]
    fo(temp, CK1)
    for i in range(16):
        W1[i] = temp[i] ^ KR[i]
        
    temp = W1[:]
    fe(temp, CK2)
    for i in range(16):
        W2[i] = temp[i] ^ W0[i]
        
    temp = W2[:]
    fo(temp, CK3)
    for i in range(16):
        W3[i] = temp[i] ^ W1[i]


    '''
    Round key generation part (ARIA-128/192/256 : 12/14/16 round, 16 bytes)
    '''
    # Generate encrypt round key (Parallel for enc_w[0, 1, 2, ...])
    rotW0 = rot(W0, 19)
    rotW1 = rot(W1, 19)
    rotW2 = rot(W2, 19)
    rotW3 = rot(W3, 19)
    for i in range(16):
        enc_w[0][i] = W0[i] ^ rotW1[i]
        enc_w[1][i] = W1[i] ^ rotW2[i]
        enc_w[2][i] = W2[i] ^ rotW3[i]
        enc_w[3][i] = rotW0[i] ^ W3[i]

    rotW0 = rot(W0, 31)
    rotW1 = rot(W1, 31)
    rotW2 = rot(W2, 31)
    rotW3 = rot(W3, 31)
    for i in range(16):
        enc_w[4][i] = W0[i] ^ rotW1[i]
        enc_w[5][i] = W1[i] ^ rotW2[i]
        enc_w[6][i] = W2[i] ^ rotW3[i]
        enc_w[7][i] = rotW0[i] ^ W3[i]

    rotW0 = rot(W0, -61)
    rotW1 = rot(W1, -61)
    rotW2 = rot(W2, -61)
    rotW3 = rot(W3, -61)
    for i in range(16):
        enc_w[8][i] = W0[i] ^ rotW1[i]
        enc_w[9][i] = W1[i] ^ rotW2[i]
        enc_w[10][i] = W2[i] ^ rotW3[i]
        enc_w[11][i] = rotW0[i] ^ W3[i]

    # Generate encrypt round key or whitening key for aria128/192/256
    rotW1 = rot(W1, -31)
    for i in range(16):
        enc_w[12][i] = W0[i] ^ rotW1[i]
        
    if keylen == 192 or keylen == 256:
        rotW2 = rot(W2, -31)
        rotW3 = rot(W3, -31)
        for i in range(16):
            enc_w[13][i] = W1[i] ^ rotW2[i]
            enc_w[14][i] = W2[i] ^ rotW3[i]
            
    if keylen == 256:
        rotW0 = rot(W0, -31)
        rotW1 = rot(W1, -19)
        for i in range(16):
            enc_w[15][i] = rotW0[i] ^ W3[i]
            enc_w[16][i] = W0[i] ^ rotW1[i]
    
    # Generate decrypt round key (Parallel for dec_w[0, 1, 2, ...])    
    for i in range(16):
        dec_w[0][i] = enc_w[round][i] # 12/14/16 for 128/192/256
        dec_w[round][i] = enc_w[0][i]

    for i in range(1,round): # 1~11
        difflayer(enc_w[round-i], dec_w[i])


'''
ARIA-128/192/256 Encrypt function
'''
def aria_enc(plaintext, ciphertext, w):
    
    round = len(w)
    state = plaintext[:]

    # 1 ~ 11/13/15 Round (128/192/256)
    for i in range(1,round-1):
        if i&1:                 # odd  round (r%2 == 1)
            fo(state, w[i-1])
        else:                   # even round
            fe(state, w[i-1])
    
    # final Round : 12/14/16 (128/192/256)
    ff(state, w[round-2], w[round-1])

    # deepcopy
    for i in range(16):
        ciphertext[i] = state[i]


'''
ARIA-128/192/256 Decrypt function
'''
def aria_dec(ciphertext, recovered, w):
    
    round = len(w)
    state = ciphertext[:]
    
    # 1 ~ 11/13/15 Round (128/192/256)
    for i in range(1, round-1):
        if i&1:
            fo(state, w[i-1])
        else:
            fe(state, w[i-1])
    
    # final Round : 12/14/16 (128/192/256)
    ff(state, w[round-2], w[round-1])
    
    # deepcopy
    for i in range(16):
        recovered[i] = state[i]
    

'''
Vraification function
'''
def verify(pt, dec):
    if pt == dec:
        return 1
    else:
        return -1
    

global round


'''
main function
'''
def main():
    global round
    
    '''
    Test Vector for ARIA-128/192/256
    '''
    pt = [0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff]
    # mk = [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f]
    # mk = [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17]
    mk = [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f]
    ct, dec = [0]*16, [0]*16
    
    '''
    Key declare part (ARIA-128/192/256)
    '''
    bits = len(mk)*8
    if bits not in [128, 192, 256]:
        print("Key length must be 128/192/256bits ")
        return
    
    if bits == 128:
        round = 12
    elif bits == 192:
        round = 14
    elif bits == 256:
        round = 16 
        
    
    '''
    Key expansion part
    '''
    enc_w = [[0 for _ in range(16)] for _ in range(round+1)] # [[0]*16]*(round+1) : 리스트 안에 리스트 mutable 안에 mutable인 경우 shallow copy
    dec_w = [[0 for _ in range(16)] for _ in range(round+1)] # [[0]*16]*(round+1) : 리스트 안에 리스트 mutable 안에 mutable인 경우 shallow copy
    keyexpansion(mk, enc_w, dec_w)
    
    
    '''
    ARIA-128/192/256 Encryption part
    '''
    aria_enc(pt, ct, enc_w)
    
    
    '''
    ARIA-128/192/256 Decryption part
    '''
    aria_dec(ct, dec, dec_w)
    
    
    
    
    '''
    Varifications part
    '''
    if verify(pt, dec) == -1:
        print('Verification failed')
        return
    
    '''
    Test vector
    '''
    print('\n ======================================================================================')
    print(' * Verification success')
    print(' ======================================================================================')
    print('                                     Encrypt key                                       ')
    print(' --------------------------------------------------------------------------------------')
    for i in range(round):
        printstate(f' * {i+1} round key = ', enc_w[i], 16)
    printstate(' * whiteningkey = ', enc_w[round], 16)
    print()
    
    
    print('\n ======================================================================================')
    print('                                     Decrypt key                                       ')
    print(' --------------------------------------------------------------------------------------')
    for i in range(round):
        printstate(f' * {i+1} round key = ', dec_w[i], 16)
    printstate(' * whiteningkey = ', dec_w[round], 16)
    print()


    print(' ======================================================================================')
    print('                                      Test Vector                                      ')
    print(' --------------------------------------------------------------------------------------')
    print(' [ ARIA-%d ]' %bits, end='\n\n')
    printstate(' * plaintext  = ', pt, 16)
    printstate(' * masterkey  = ', mk, len(mk))
    printstate(' * ciphertext = ', ct, 16)

    print(' --------------------------------------------------------------------------------------')
    print(' [ ARIA-%d ]' %bits, end='\n\n')
    printstate(' * ciphertext = ', ct, 16)
    printstate(' * masterkey  = ', mk, len(mk))
    printstate(' * recovered  = ', dec, 16)
    print(' ======================================================================================')
    
        
    
if __name__ == "__main__":
    main()

# EOF