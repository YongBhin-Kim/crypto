...
public class KISA_SEED_ECB {
	...
	public static byte[] SEED_ECB_Encrypt(byte[] pbszUserKey, byte[] pbData, int offset, int length) {
		...
		int nPlainTextPadding = (BLOCK_SIZE_SEED - (nPlainTextLen) % BLOCK_SIZE_SEED); // 패딩크기는 평문크기가 16이므로 16으로 설정됨
		byte[] newpbszPlainText = new byte[nPlainTextLen + nPlainTextPadding]; // 해당 변수는 기존 평문크기 + 패딩 크기로 해서 블록 사이즈의 배수로 설정됨
		
		
		Common.arraycopy(newpbszPlainText, pbszPlainText, nPlainTextLen); // newpbszPlainText <- pbszPlainText || 00...0 (평문 16바이트 기준 32바이트짜리 배열)
		
		byte[] pbszCipherText = new byte[nPlainTextLen + nPlainTextPadding];

		SEED_ECB_init( info, KISA_ENC_DEC.KISA_ENCRYPT, pbszUserKey);
		// System.out.print("pbszCipherText: ");
		// for (byte b : pbszCipherText) System.out.printf("%02X ", b); System.out.println();

		
		outlen =((newpbszPlainText.length/BLOCK_SIZE_SEED) )*BLOCK_SIZE_SEED_INT ; // ct길이: char기준 32 -> int기준 8
		
		outbuf = new int[outlen]; // int 배열 ct 담을 버퍼 선언
	...
	*/
	...
	public static void main(String[] args)
	{
		...
		byte pbUserKey[] = {(byte)0x01, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, 
		...
		byte pbData1[] = {(byte)0x02, (byte)0x01, (byte)0x02, (byte)0x03, (byte)0x04, (byte)0x05, (byte)0x06, (byte)0x07, (byte)0x08, (byte)0x09, (byte)0x0A, (byte)0x0B, (byte)0x0C, (byte)0x0D, (byte)0x0E, (byte)0x0F};
		byte pbData2[]     = {(byte)0x03, (byte)0x01, (byte)0x02, (byte)0x03, (byte)0x04, (byte)0x05, (byte)0x06, (byte)0x07, (byte)0x08, (byte)0x09, (byte)0x0A, (byte)0x0B, (byte)0x0C, (byte)0x0D, (byte)0x0E, (byte)0x0F, (byte)0x00, (byte)0x01};
		...
		System.out.print("Plaintext\t: "); for (int i=0; i<16; i++)	System.out.print(Integer.toHexString(0xff&pbData[i])+" "); System.out.print("\n\n\n");
	    
	    pbCipher = SEED_ECB_Encrypt(pbUserKey, pbData, 0, 16);
	    pbPlain = SEED_ECB_Decrypt(pbUserKey, pbCipher, 0, 32);
		
		System.out.print("Ciphertext(SEED_ECB_Encrypt)\t: "); for (int i=0; i<32; i++)	System.out.print(Integer.toHexString(0xff&pbCipher[i])+" "); System.out.print("\n");
	    System.out.print("Plaintext(SEED_ECB_Decrypt)\t: "); for (int i=0; i<16; i++)	System.out.print(Integer.toHexString(0xff&pbPlain[i])+" "); System.out.print("\n\n\n");
	...
	//     pbCipher = SEED_ECB_Encrypt(pbUserKey, pbData1, 0, pbData1.length);
	// 	System.out.println("pb2 length = " + pbData1.length);
	...
	//     for (int i=0; i<pbData1.length; i++)	System.out.print(Integer.toHexString(0xff&pbPlain[i])+" ");
	//     System.out.print("\n\n");
