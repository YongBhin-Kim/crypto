...
private static void UpdateCounter_for_SEED( int[] pbOUT, int pbOUT_offset, int nIncreaseValue, int nMin ) {
	...
	if( 0 < nMin ) {
		byte b = get_byte_for_int(pbOUT, pbOUT_offset*4+nMin, ENDIAN);
		bszBackup = b & 0x0ff;
		set_byte_for_int(pbOUT, pbOUT_offset*4+nMin, (byte)(b + nIncreaseValue), ENDIAN);
	}

	for( i=nMin; i>1; --i )
	{
		if( bszBackup <= (((int)get_byte_for_int(pbOUT, pbOUT_offset*4+i, ENDIAN)) & 0x0ff) ) {
			return;
		}
		else {
			byte b = get_byte_for_int(pbOUT, pbOUT_offset*4+i-1, ENDIAN);
			bszBackup = b & 0x0ff;
			set_byte_for_int(pbOUT, pbOUT_offset*4+i-1, (byte)(b + 1), ENDIAN);
		}
	}

	byte b = get_byte_for_int(pbOUT, pbOUT_offset*4+0, ENDIAN);
	bszBackup = b & 0x0ff;
	set_byte_for_int(pbOUT, pbOUT_offset*4+0, (byte)(b + nIncreaseValue), ENDIAN);
}

/**
@brief byte ë°°ì´ì int ë°°ì´ë¡ ë³ííë¤.
@param in :ë³íí  byte í¬ì¸í°
@param nLen : ë³íí  byte ë°°ì´ ê°¯ì
@return ì¸ìë¡ ë°ì byte ë°°ì´ì intë¡ ë³íë í¬ì¸í°ë¥¼ ë°ííë¤.
 */
...
/**
@brief int ë°°ì´ì byte ë°°ì´ë¡ ë³ííë¤.
@param in :ë³íí  int í¬ì¸í°
@param nLen : ë³íí  int ë°°ì´ ê°¯ì
@return ì¸ìë¡ ë°ì int ë°°ì´ì byteë¡ ë³íí í¬ì¸í°ë¥¼ ë°ííë¤.
 */
...
/**
@brief SEED CTR ìê³ ë¦¬ì¦ ì´ê¸°í í¨ì
@param pInfo : CTR ë´ë¶ìì ì¬ì©ëë êµ¬ì¡°ì²´ë¡ì¨ ì ì ê° ë³ê²½íë©´ ìëë¤.(ë©ëª¨ë¦¬ í ë¹ëì´ ìì´ì¼ íë¤.
)
@param enc : ìí¸í ë° ë³µí¸í ëª¨ë ì§ì 
@param pszUserKey : ì¬ì©ìê° ì§ì íë ìë ¥ í¤(16 byte)
@param pbszCounter : ì¬ì©ìê° ì§ì íë ì´ê¸°í ë²¡í°(16 byte)
*/
public static int SEED_CTR_init( KISA_SEED_INFO pInfo, KISA_ENC_DEC enc, byte[] pszUserKey, byte[] pbszCounter ) {
	int ABCD[] = new int[4];
	int T[] = new int[2];
	int K[];
	...
	memcpy(pInfo.ctr, pbszCounter, 16, ENDIAN);
	pInfo.last_block_flag = pInfo.buffer_length = 0;

	ABCD[ABCD_A] = byte_to_int(pszUserKey, 0*4, ENDIAN);
	ABCD[ABCD_B] = byte_to_int(pszUserKey, 1*4, ENDIAN);
	ABCD[ABCD_C] = byte_to_int(pszUserKey, 2*4, ENDIAN);
	ABCD[ABCD_D] = byte_to_int(pszUserKey, 3*4, ENDIAN);
...
/**
@brief SEED CTR ë¤ì¤ ë¸ë­ ìí¸í/ë³µí¸í í¨ì
@param pInfo : SEED_CTR_init ìì ì¤ì ë KISA_HIGHT_INFO êµ¬ì¡°ì²´
@param in : íë¬¸/ìí¸ë¬¸ ( íë¬¸ì chartoint32_for_SEED_CTRë¥¼ ì¬ì©íì¬ intë¡ ë³íë ë°ì´í°)
@param inLen : íë¬¸/ìí¸ë¬¸ ê¸¸ì´(byte ë¨ì)
@param out : íë¬¸/ìí¸ë¬¸ ë²í¼
@param outLen : ì§íë íë¬¸/ìí¸ë¬¸ì ê¸¸ì´(byte ë¨ìë¡ ëê²¨ì¤ë¤)
@return 0: inLenì ê°ì´ 0ë³´ë¤ ìì ê²½ì°, KISA_SEED_INFO êµ¬ì¡°ì²´ë in, outì ë í¬ì¸í°ê° í ë¹ëìì ê²½ì°
        1: ì±ê³µ
*/
...
public static int SEED_CTR_Process( KISA_SEED_INFO pInfo, int[] in, int inLen, int[] out, int[] outLen ) {
	...
	while( nCurrentCount < inLen )
	{
		...
		UpdateCounter_for_SEED( pbszCounter, pbszCounter_offset, 1, (BLOCK_SIZE_SEED-1) );
...
public static int SEED_CTR_Close( KISA_SEED_INFO pInfo, int[] out, int out_offset, int[] outLen ) {
	int nEncRmainLeng = -(pInfo.buffer_length);
	int i;
	...
	outLen[0] = nEncRmainLeng;
	return 1;
}

/**
@brief ì²ë¦¬íê³ ì íë ë°ì´í°ê° ì ì ê²½ì°ì ì¬ì©
@param pbszUserKey : ì¬ì©ìê° ì§ì íë ìë ¥ í¤(16 byte)
@param pszbIV : ì¬ì©ìê° ì§ì íë ì´ê¸°í ë²¡í°(16 byte)
@param pbszPlainText : ì¬ì©ì ìë ¥ íë¬¸
@param nPlainTextLen : íë¬¸ ê¸¸ì´(byte ë¨ìì íë¬¸ê¸¸ì´)
@return ìí¸íë ê²°ê³¼ byte
*/
public static byte[] SEED_CTR_Encrypt( byte[] pbszUserKey, byte[] pbszCTR, byte[] message, int message_offset, int nInputTextLen ) {
	int nOutLeng[] = { 0 };
	...
	byte []pbszInputText = new byte[nInputTextLen];
	arraycopy_system(message, message_offset, pbszInputText, 0, nInputTextLen);
	
	int nInputTextPadding = (BLOCK_SIZE_SEED - (nInputTextLen % BLOCK_SIZE_SEED)) % BLOCK_SIZE_SEED;
	byte []newpbszInputText = new byte[nInputTextLen + nInputTextPadding];
	arraycopy(newpbszInputText, pbszInputText, nInputTextLen);

	byte[] pbszOutputText = new byte[nInputTextLen];
		
	SEED_CTR_init( info, KISA_ENC_DEC.KISA_ENCRYPT, pbszUserKey, pbszCTR );

	outlen = ((newpbszInputText.length/16)) * 4;
	...
	SEED_CTR_Close( info, outbuf, nOutLeng[0], nEncRmainLeng );
	cdata = int32tochar_for_SEED_CTR(outbuf, nOutLeng[0] - nEncRmainLeng[0] );
	arraycopy(pbszOutputText, cdata, nOutLeng[0] - nEncRmainLeng[0] );
...
public static byte[] SEED_CTR_Decrypt( byte[] pbszUserKey, byte[] pbszCTR, byte[] message, int message_offset, int nInputTextLen ) {
	int nOutLeng[] = { 0 };
	int nEncRmainLeng[] = new int[] { 0 };
	KISA_SEED_INFO info = new KISA_SEED_INFO();
	int[] outbuf;
