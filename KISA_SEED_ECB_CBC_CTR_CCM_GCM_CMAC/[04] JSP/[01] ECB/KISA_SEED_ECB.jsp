...
public static void KISA_SEED_Encrypt_Block_forECB( int[] in, int in_offset, int[] out, int out_offset, KISA_SEED_KEY ks ) {
...
public static int SEED_ECB_Process( KISA_SEED_INFO pInfo, int[] in, int inLen, int[] out, int[] outLen ) {
	...
	if( KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt ) {
		...
		memcpy( pInfo.ecb_buffer, in, in_offset, pInfo.buffer_length );
	}
	else {
		in_offset = 0;
		out_offset = 0;
		...
		outLen[0] = nCurrentCount - BLOCK_SIZE_SEED;
		memcpy( pInfo.ecb_last_block, out, out_offset - 4, BLOCK_SIZE_SEED );
	}
...
public static int SEED_ECB_Close( KISA_SEED_INFO pInfo, int[] out, int out_offset, int[] outLen ) {
	...
	if( KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt ) {
		...
		for( i = pInfo.buffer_length; i<BLOCK_SIZE_SEED; i++ ) {
			set_byte_for_int(pInfo.ecb_buffer, i, (byte)nPaddngLeng, ENDIAN);
		}
		KISA_SEED_Encrypt_Block_forECB( pInfo.ecb_buffer, 0, out, (out_offset)/4, pInfo.seed_key );
		outLen[0] = BLOCK_SIZE_SEED;
	}
	else {
		nPaddngLeng = get_byte_for_int(pInfo.ecb_last_block, BLOCK_SIZE_SEED-1, ENDIAN);
...
/**
@brief Data를 암호화한 결과 byte 반환
@param pbszUserKey : 사용자 입력키
@param pbData : 암호화할 데이터
@param offset : 암호화할 데이터 offset
@param length : 암호화할 데이터 길이
@return pbData를 암호화한 결과 byte
*/
public static byte[] SEED_ECB_Encrypt(byte[] pbszUserKey, byte[] pbData, int offset, int length) {
	...
	arraycopy_system(pbData, offset, pbszPlainText, 0, length);
